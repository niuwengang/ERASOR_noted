...
class OfflineMapUpdater
{
  private:
    ...
    double query_voxel_size_; // 当前帧滤波
    double map_voxel_size_;   // 子图滤波
    /**< ERASOR does not conduct dynamic object removal at all time steps!
     * removal_interval needs some heursitics */
    int removal_interval_;           // 去除动态障碍物算法间隔
    int global_voxelization_period_; //! 暂时未知
    bool verbose_;                   // 是否输出详细信息
    bool is_large_scale_;            // 是否大尺寸场景
    bool is_submap_not_initialized_ = true;

    /**< Params. of Volume of Interest (VoI) */
    double max_range_;   // voi半径
    double min_h_;       // voi最低高度
    double max_h_;       // voi最高高度
    double submap_size_; //! submap个数
     ...
     */
    pcl::PointCloud<pcl::PointXYZI>::Ptr map_init_; // 原始pcd点云
    ...
    Eigen::Matrix4f tf_lidar2body_;//雷达系统到车体系 /**< Transformation matrix between the initial pose to the origin */
    ...
    void set_submap(const pcl::PointCloud<pcl::PointXYZI> &map_global, pcl::PointCloud<pcl::PointXYZI> &submap,
                    pcl::PointCloud<pcl::PointXYZI> &submap_complement, double x, double y, double submap_size);

    void fetch_VoI(double x_criterion, double y_criterion, pcl::PointCloud<pcl::PointXYZI> &dst,
                   pcl::PointCloud<pcl::PointXYZI> &outskirts, std::string mode = "naive");

    void print_status();

    void set_path(nav_msgs::Path &path, std::string mode, const erasor::node &node,
