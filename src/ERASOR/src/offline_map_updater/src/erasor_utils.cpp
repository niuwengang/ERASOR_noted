...
geometry_msgs::Pose eigen2geoPose(Eigen::Matrix4f pose)
{
    ...
    tf::Matrix3x3 m;
    m.setValue((double)pose(0, 0), (double)pose(0, 1), (double)pose(0, 2), (double)pose(1, 0), (double)pose(1, 1),
               (double)pose(1, 2), (double)pose(2, 0), (double)pose(2, 1), (double)pose(2, 2));
...
void parse_dynamic_obj(const pcl::PointCloud<pcl::PointXYZI> &cloudIn, pcl::PointCloud<pcl::PointXYZI> &dynamicOut,
                                ...
                                double leaf_size)
{
     ...
     * so this function is to conduct voxelization followed by nearest points search to re-assign the label of each
     * point */
    /*体素滤波后，强度用最近距离点的强度替代*/
    pcl::PointCloud<pcl::PointXYZI>::Ptr ptr_voxelized(new pcl::PointCloud<pcl::PointXYZI>); // 滤波后的点云
    ...
    ptr_reassigned->points.reserve(ptr_voxelized->points.size()); // 按滤波后的预分配内存

    int K = 1; // 最近邻点数1

    std::vector<int> pointIdxNKNSearch(K);         // 最近邻下标
    std::vector<float> pointNKNSquaredDistance(K); // 最近邻距离
